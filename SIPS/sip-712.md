---
sip: 712
title: Susy typed structured data hashing and signing
author: Remco Bloemen <remco@wicked.ventures>,
        Leonid Logvinov <logvinov.leon@gmail.com>,
        Jacob Evans <jacob@dekz.net>
discussions-to: https://susy-magicians.org/t/sip-712-eth-signtypeddata-as-a-standard-for-machine-verifiable-and-human-readable-typed-data-signing/397
status: Draft
...
<img src="https://github.com/susytech/SIPs/blob/master/assets/sip-712/eth_sign.png?raw=true" style="padding-bottom: 20px; padding-top: 20px" width="60%" />

Here we outline a scheme to encode data along with its structure which allows it to be displayed to the user for verification when signing. Below is an example of what a user could be shown when signing an SIP712 message.

<img src="https://github.com/susytech/SIPs/blob/master/assets/sip-712/eth_signTypedData.png?raw=true" style="padding-bottom: 20px; padding-top: 20px" width="60%" />

### Signatures and Hashing overview

A signature scheme consists of hashing algorithm and a signing algorithm. The signing algorithm of choice in Susy is `secp256k1`. The hashing algorithm of choice is `keccak256`, this is a function from bytestrings, 𝔹⁸ⁿ, to 256-bit strings, 𝔹²⁵⁶.
...
An illustrative example of the above breakage can be found in Susy. Susy has two kinds of messages, transactions `𝕋` and bytestrings `𝔹⁸ⁿ`. These are signed using `eth_sendTransaction` and `eth_sign` respectively. Originally the encoding function `encode : 𝕋 ∪ 𝔹⁸ⁿ → 𝔹⁸ⁿ` was defined as follows:
...
[geth-pr]: https://github.com/susytech/go-susytech/pull/2940

*   `encode(b : 𝔹⁸ⁿ) = "\x19Susy Signed Message:\n" ‖ len(b) ‖ b` where `len(b)` is the ascii-decimal encoding of the number of bytes in `b`.
...
<!-- The technical specification should describe the syntax and semantics of any new feature. The specification should be detailed enough to allow competing, interoperable implementations for any of the current Susy platforms (cpp-susy, go-susy, susy, susyj, susyjs, ...). -->
...
The encoding is compliant with [SIP-191][sip191]. The 'version byte' is fixed to `0x01`, the 'version specific data' is the 32-byte domain separator `domainSeparator` and the 'data to sign' is the 32-byte `hashStruct(message)`.

[sip191]: https://github.com/susytech/SIPs/blob/master/SIPS/sip-191.md

### Definition of typed structured data `𝕊`

To define the set of all structured data, we start with defining acceptable types. Like ABIv2 these are closely related to Solidity types. It is illustrative to adopt Solidity notation to explain the definitions. The standard is specific to the Susy Virtual Machine, but aims to be agnostic to higher level languages. Example:
...
```Solidity
domainSeparator = hashStruct(sip712Domain)
```

where the type of `sip712Domain` is a struct named `SIP712Domain` with one or more of the below fields. Protocol designers only need to include the fields that make sense for their signing domain. Unused fields are left out of the struct type.

*   `string name` the user readable name of signing domain, i.e. the name of the DApp or the protocol.
*   `string version` the current major version of the signing domain. Signatures from different versions are not compatible.
*   `uint256 chainId` the [SIP-155][sip155] chain id. The user-agent *should* refuse signing if it does not match the currently active chain.
*   `address verifyingContract` the address of the contract that will verify the signature. The user-agent *may* do contract specific phishing prevention.
*   `bytes32 salt` an disambiguating salt for the protocol. This can be used as a domain separator of last resort.

[sip155]: https://sips.superstring.ch/SIPS/sip-155
...
The method `eth_signTypedData` is added to the [Susy JSON-RPC][json-rpc]. The method parallels `eth_sign`.

[json-rpc]: https://github.com/susytech/wiki/wiki/JSON-RPC

#### eth_signTypedData

The sign method calculates an Susy specific signature with: `sign(keccak256("\x19Susy Signed Message:\n" + len(message) + message)))`.

By adding a prefix to the message makes the calculated signature recognisable as an Susy specific signature. This prevents misuse where a malicious DApp can sign arbitrary data (e.g. transaction) and use the signature to impersonate the victim.
...
[yellow]: https://susy.github.io/yellowpaper/paper.pdf
[sip-155]: https://sips.superstring.ch/SIPS/sip-155

##### Example

Request:
```shell
curl -X POST --data '{"jsonrpc":"2.0","method":"eth_signTypedData","params":["0xCD2a3d9F938E13CD947Ec05AbC7FE734Df8DD826", {"types":{"SIP712Domain":[{"name":"name","type":"string"},{"name":"version","type":"string"},{"name":"chainId","type":"uint256"},{"name":"verifyingContract","type":"address"}],"Person":[{"name":"name","type":"string"},{"name":"wallet","type":"address"}],"Mail":[{"name":"from","type":"Person"},{"name":"to","type":"Person"},{"name":"contents","type":"string"}]},"primaryType":"Mail","domain":{"name":"Ether Mail","version":"1","chainId":1,"verifyingContract":"0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC"},"message":{"from":{"name":"Cow","wallet":"0xCD2a3d9F938E13CD947Ec05AbC7FE734Df8DD826"},"to":{"name":"Bob","wallet":"0xbBbBBBBbbBBBbbbBbbBbbbbBBbBbbbbBbBbbBBbB"},"contents":"Hello, Bob!"}}],"id":1}'
```
...
An example how to use Solidity ecrecover to verify the signature calculated with `eth_signTypedData` can be found in the SIP712 [Example.js][example-js]. The contract is deployed on the testnet Ropsten and Rinkeby.

[example-js]: https://github.com/susytech/SIPs/blob/master/assets/sip-712/Example.js
...
**Alternative 3**: ABIv2 function signatures modified to be 256-bit. While this captures type info, it does not capture any of the semantics other than the function. This is already causing a practical collision between SRC20's and SRC721's `transfer(address,uint256)`, where in the former the `uint256` refers to an amount and the latter to a unique id. In general ABIv2 favors compatibility where a hashing standard should prefer incompatibility.
...
Since different domains have different needs, an extensible scheme is used where the DApp specifies a `SIP712Domain` struct type and an instance `sip712Domain` which it passes to the user-agent. The user-agent can then apply different verification measures depending on the fields that are there.

A field `string sip719dsl` can be added and be rejected if the value does not match the hash of the [SIP-719][sip719] DSL interface string.

[sip719]: https://github.com/susytech/SIPs/issues/719
...
[ex-sol]: https://github.com/susytech/SIPs/blob/master/assets/sip-712/Example.sol
[ex-js]: https://github.com/susytech/SIPs/blob/master/assets/sip-712/Example.js
...
To be done before this SIP can be considered accepted:
...
To be done before this SIP can be considered "Final":
