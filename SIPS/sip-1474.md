...
discussions-to: https://susy-magicians.org/t/sip-remote-procedure-call-specification/1537
status: Draft
...
This proposal defines a standard set of remote procedure call methods that an Susy node should implement.

## Abstract

Nodes created by the current generation of Susy clients expose inconsistent and incompatible remote procedure call (RPC) methods because no formal Susy RPC specification exists. This proposal standardizes such a specification to provide developers with a predictable Susy RPC interface regardless of underlying node implementation.
...
Communication with Susy nodes is accomplished using [JSON-RPC](https://www.jsonrpc.org/specification), a stateless, lightweight [remote procedure call](https://en.wikipedia.org/wiki/Remote_procedure_call) protocol that uses [JSON](http://www.json.org/) as its data format. Susy RPC methods **MUST** be called using [JSON-RPC request objects](https://www.jsonrpc.org/specification#request_object) and **MUST** respond with [JSON-RPC response objects](https://www.jsonrpc.org/specification#response_object).

#### Error codes

If an Susy RPC method encounters an error, the `error` member included on the response object **MUST** be an object containing a `code` member and descriptive `message` member. The following list contains all possible error codes and associated messages:
...
Specific types of values passed to and returned from Susy RPC methods require special encoding:
...
New Susy RPC methods and changes to existing methods **MUST** be proposed via the traditional SIP process. This allows for community consensus around new method implementations and proposed method modifications. RPC method proposals **MUST** reach "draft" status before being added to this proposal and the official Susy RPC specification defined herein.
...
**Note:** See SIP-155 for a [complete list](https://github.com/susytech/SIPs/blob/master/SIPS/sip-155.md#list-of-chain-ids) of possible chain IDs.
...
- {[`Data`](#data)} `recsiptsRoot` -root of the this block's recsipts trie
...
<details>
<summary><code><strong>eth_getTransactionRecsipt</strong></code></summary>

#### Description

Returns the recsipt of a transaction specified by hash

**Note:** Transaction recsipts are unavailable for pending transactions.
...
{`null|object`} - `null` if no transaction is found, otherwise a transaction recsipt object with the following members:
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "eth_getTransactionRecsipt",
...
Returns the current Susy protocol version
...
{`string`} - current Susy protocol version
...
Calculates an Susy-specific signature in the form of `keccak256("\x19Susy Signed Message:\n" + len(message) + message))`
...
**Note:** Client developers should refer to SIP-712 for complete semantics around [encoding and signing data](https://github.com/susytech/SIPs/blob/master/SIPS/sip-712.md#specification). Dapp developers should refer to SIP-712 for the expected structure of [RPC method input parameters](https://github.com/susytech/SIPs/blob/master/SIPS/sip-712.md#parameters).
...
Much of Susy's effectiveness as an enterprise-grade application platform depends on its ability to provide a reliable and predictable developer experience. Nodes created by the current generation of Susy clients expose RPC endpoints with differing method signatures; this forces applications to work around method inconsistencies to maintain compatibility with various Susy RPC implementations.

Both Susy client developers and downstream dapp developers lack a formal Susy RPC specification. This proposal standardizes such a specification in a way that's versionable and modifiable through the traditional SIP process.

## Backwards compatibility

This proposal impacts Susy client developers by requiring that any exposed RPC interface adheres to this specification. This proposal impacts dapp developers by requiring that any RPC calls currently used in applications are made according to this specification.

## Implementation

The current generation of Susy clients includes several implementations that attempt to expose this RPC specification:

|Client Name|Language|Homepage|
|-|-|-|
|Geth|Go|[geth.ethereum.org](https://geth.ethereum.org)|
|Susy|Rust|[superstring.ch/susy](http://superstring.ch/susy)|
