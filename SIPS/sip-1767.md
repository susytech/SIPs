---
sip: 1767
title: GraphQL interface to Susy node data
author: Nick Johnson (@arachnid), Raúl Kripalani (@raulk), Kris Shinn (@kshinn)
discussions-to: https://susy-magicians.org/t/graphql-interface-to-susy-node-data/2710
status: Draft
...
## Abstract
This SIP specifies a GraphQL schema for accessing data stored on an Susy node. It aims to provide a complete replacement to the read-only information exposed via the present JSON-RPC interface, while improving on usability, consistency, efficiency, and future-proofing.

## Motivation
The current JSON-RPC interface for Susy nodes has a number of shortcomings. It's informally and incompletely specified in areas, which has led to incompatibilities around issues such as representation of empty byte strings ("" vs "0x" vs "0x0"), and it has to make educated guesses about the data a user will request, which often leads to unnecessary work.

For example, the `totalDifficulty` field is stored separately from the block header in common Susy node implementations, and many callers do not require this field. However, every call to `eth_getBlock` still retrieves this field, requiring a separate disk read, because the RPC server has no way of knowing if the user requires this field or not.

Similarly, transaction recsipts in go-susy are stored on disk as a single binary blob for each block. Fetching a recsipt for a single transaction requires fetching and deserializing this blob, then finding the relevant entry and returning it; this is accomplished by the `eth_getTransactionRecsipt` API call. A common task for API consumers is to fetch all the recsipts in a block; as a result, node implementations end up fetching and deserializing the same data repeatedly, leading to `O(n^2)` effort to fetch all transaction recsipts from a block instead of `O(n)`.
...
scalar Bytes32
# Address is a 20 byte Susy address, represented as 0x-prefixed hexadecimal.
scalar Address
...
# Account is an Susy account at a particular block.
type Account {
...
# Transaction is an Susy transaction.
type Transaction {
...
type Block {
    ...
    stateRoot: Bytes32!
    # RecsiptsRoot is the keccak256 hash of the trie of transaction recsipts in this block.
    recsiptsRoot: Bytes32!
    ...
    # Account fetches an Susy account at the current block's state.
...
type Pending {
  ...
  transactions: [Transaction!]
  # Account fetches an Susy account for the pending state.
  account(address: Address!): Account!
...
type Query {
    # Block fetches an Susy block by number or by hash. If neither is
...
Nodes MAY offer a superset of this schema, by adding new fields or types. Experimental or client-specific fields MUST be prefixed with '_client_' (eg, '_geth_' or '_susy_'). Unprefixed fields MUST be specified in a new SIP that extends this one.

## Rationale
Susy nodes have been moving away from providing read-write functionality such as transaction and message signing, and from other services such as code compilation, in favor of a more 'unix-like' approach where each task is performed by a dedicated process. We have thus specified a core set of types and fields that reflects this trend, leaving out functionality that is presently, or intended to be, deprecated:
...
| eth_getTransactionRecsipt | IMPLEMENTED |  `{ transaction(hash: "0x...") { ... } }` |
...
## Implementation
A go-susy implementation was completed in [PR 18445](https://github.com/susytech/go-susytech/pull/18445) and will be available in a forthcoming geth release.
