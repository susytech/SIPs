...
discussions-to: https://github.com/susytech/SIPs/pulls/1820
status: Final
...
Interfaces with zeroes (`0`) as the last 28 bytes are considered [SRC165] interfaces,
...
There have been different approaches to define pseudo-introspection in Susy.
The first is [SRC165] which has the limitation that it cannot be used by regular accounts.
The second attempt is [SRC672] which uses reverse [ENS]. Using reverse [ENS] has two issues. 
...
### [SRC1820] Registry Smart Contract
...
/// @notice For more details, see https://sips.superstring.ch/SIPS/sip-1820
...
<code>{
        ...
        "output": {
          ...
          "userdoc": {
            ...
            "notice": "This contract is the official implementation of the SRC1820 Registry.For more details, see https://sips.superstring.ch/SIPS/sip-1820"
          }
        },
        "settings": {
          "compilationTarget": {
...
If the interface is part of a standard, it is best practice to explicitly state the interface name and link to this published [SRC1820] such that other people don't have to come here to look up these rules.
...
If a contract dynamically changes its interface and relies on the [SRC165] cache of the [SRC1820] registry, the cache MUST be updated manually---there is no automatic cache invalidation or cache update. 
...
For any address to set a contract as the interface implementation, it must call the following function of the [SRC1820] registry:
...
Anyone MAY query the [SRC1820] Registry to obtain the address of a contract implementing an interface on behalf of some address using the `getInterfaceImplementer` function.
...
*NOTE*: If the last 28 bytes of the `_interfaceHash` are zeroes (`0`), then the first 4 bytes are considered an [SRC165] interface and the registry SHALL forward the call to the contract at `_addr` to see if it implements the [SRC165] interface (the first 4 bytes of `_interfaceHash`). 
The registry SHALL also cache [SRC165] queries to reduce gas consumption. Anyone MAY call the `src165UpdateCache` function to update whether a contract implements an interface or not.
...
The registry can also act as a [SRC165] cache in order to save gas when looking up if a contract implements a specific [SRC165] interface. 
