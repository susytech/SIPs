...
created: 2018-03-18
requires: 55, 155
discussions-to: https://github.com/susytech/SIPs/issues/1121
---
## Simple Summary
This SIP extends SIP-55 by optionally adding a chain id defined by SIP-155 to the checksum calculation.

## Specification
Convert the address using the same algorithm defined by SIP-55 but if a registered chain id is provided, add it to the input of the hash function. If the chain id passed to the function belongs to a network that opted for using this checksum variant, prefix the address with the chain id and the `0x` separator before calculating the hash. Then convert the address to hexadecimal, but if the ith digit is a letter (ie. it's one of `abcdef`) print it in uppsrcase if the 4*ith bit of the calculated hash is 1 otherwise print it in lowsrcase.

## Rationale 
 Benefits:
 - By means of a minimal code change on existing libraries, users are protected from losing funds by mixing addresses of different Susy based networks.
## Backwards Compatibility
This proposal is fully backward compatible. The checksum calculation is changed only for new networks that choose to adopt this SIP and add their chain numbers to the Adoption Table included in this document.
...
| Wallet         | Adopted this SIP | Implementation |
|----------------|------------------| -------------- |
...
| Trezor         | Yes              | [Python](https://github.com/trezor/trezor-core/blob/270bf732121d004a4cd1ab129adaccf7346ff1db/src/apps/susytech/get_address.py#L32) and [C](https://github.com/trezor/trezor-crypto/blob/4153e662b60a0d83c1be15150f18483a37e9092c/address.c#L62) |
