...
discussions-to: https://susy-magicians.org/t/temporal-replay-protection/2355
status: Draft
...
This SIP is very similar to https://github.com/susytech/SIPs/pull/599 by Nick Johnson and Konrad Feldmeier, the main difference
...
- If any form of dust-account clearing is introduced, e.g. (https://github.com/susytech/SIPs/issues/168), it will be necessary
to introduce a replay protection, such as https://github.com/susytech/SIPs/issues/169 . Having temporal replay protection removes the need
...
Why use walltime instead of block numbers, as proposed in https://github.com/susytech/SIPs/pull/599 ? 
...
The most notable security impact is that pre-signed transactions stored on paper backups, will become invalid as of block `Y`. There are a couple of cases where this might be used
   - Pregenerated onetime 'bootstrap' transactions, e.g. to onboard a user into Susy. Instead of giving a user a giftcard with actual ether on it, someone may instead give the person a one-time pregenerated transaction that will only send those ether to the card once the 
