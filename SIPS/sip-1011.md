...
Specification of the first step to transition Susy main net from Proof of Work (PoW) to Proof of Stake (PoS). The resulting consensus model is a PoW/PoS hybrid.

## Abstract

This SIP specifies a hybrid PoW/PoS consensus model for Susy main net. Existing PoW mechanics are used for new block creation, and a novel PoS mechanism called Casper the Friendly Finality Gadget (FFG) is layered on top using a smart contract.
...
This SIP does not contain safety and liveness proofs or validator implementation details, but these can be found in the [Casper FFG paper](https://arxiv.org/abs/1710.09437) and [Validator Implementation Guide](https://github.com/susytech/casper/blob/master/VALIDATOR_GUIDE.md) respectively.
...
* When applying `vote` transactions to `CASPER_ADDR` to vm state:
  ...
  * Do not count gas of `vote` toward the normal `gas_used`. For all `vote` transaction recsipts, cumulative gas used is equal to last non-`vote` transaction recsipt
...
See the [Validator Implementation Guide](https://github.com/susytech/casper/blob/master/VALIDATOR_GUIDE.md) for validator details.

#### MSG_HASHER_CODE

The source code for `MSG_HASHER_CODE` is located [here](https://github.com/susytech/casper/blob/master/casper/contracts/msg_hash.se.py).
...
The source code for `PURITY_CHECKER_CODE` is located [here](https://github.com/susytech/research/blob/master/impurity/check_for_impurity.se).
The source is to be migrated to Vyper LLL before the bytecode is finalized for this SIP.
...
[here](https://github.com/susytech/casper/blob/master/casper/contracts/simple_casper.v.py).
...
For more details on validator account abstraction, see the [Validator Implementation Guide](https://github.com/susytech/casper/blob/master/VALIDATOR_GUIDE.md).
