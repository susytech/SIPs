...
discussions-to: https://susy-magicians.org/t/src-proposal-ldgrtoken-a-compatible-security-token-for-issuing-and-trading-sec-compliant-securities/1468
status: Draft
...
There are currently no token standards that expressly facilitate conformity to securities law and related regulations. SRC-20 tokens do not support the regulated roles of Funding Portal, Broker Dealer, RTA, and Investor and do not support the [Bank Secrecy Act/USA Patriot Act KYC and AML requirements](https://www.occ.treas.gov/topics/compliance-bsa/bsa/index-bsa.html). Other improvements (notably [SIP-1404 (Simple Restricted Token Standard)](https://github.com/susytech/SIPs/issues/1404) have tried to tackle KYC and AML regulatory requirement. This approach is novel because the RTA is solely responsible for performing KYC and AML and should be solely responsible for `transferFrom`, `mint`, and `burnFrom`.

## Specification
`SRC-1450` extends `SRC-20`.

### `SRC-1450`
`SRC-1450` requires that only the Issuer can create a token representing the security that only the RTA manages. Instantiating the `SRC-1450` requires the `Owned` and `IssuerControlled` modifiers, and only the Issuer should execute the `SRC-1450` constructor for a compliant token. `SRC-1450` extends the general `Ownable` modifier to describe a specific subset of owners that automate and decentralize compliance through the contract modifiers `Owned` and `IssuerControlled` and the function modifiers `onlyOwner` and `onlyIssuerTransferAgent`. The `Owned` contract modifier instantiates the `onlyOwner` modifier for functions. The `IssuerControlled` modifier instantiates the `onlyIssuerTransferAgent` modifier for functions.
...
#### Issuer Controlled
`IssuerControlled` maintains the Issuer’s ownership of their securities by owning the contract and enables the Issuer to set and update the RTA for the Issuer’s securities. `SRC-1450`‘s constructor must have an `IssuerControlled` modifier with the issuer specified in its `SRC-1450` constructor. `IssuerControlled` instantiates the `onlyIssuerTransferAgent` modifier for `SRC-1450` to enable specific functions (`setPhysicalAddressOfOperation` and `setTransferAgent`) to permit only the Issuer to execute these functions.
...
Contract SRC-1450 is Owned, IssuerControlled {
...
Implementers must maintain off-chain services and databases that record and track the Investor’s name, physical address, Susy address, and security ownership amount. The implementers and the SEC must be able to access the Investor’s private information on an as needed basis. Issuers and the RTA must be able to produce a current list of all Investors, including the names, addresses, and security ownership levels for every security at any given moment. Issuers and the RTA must be able to re-issue securities to Investors for a variety of regulated reasons.
...
If the RTA loses access, the RTA can create a new Susy address, and the Issuer can execute the `setTransferAgent` function to reassign the RTA.
...
## Rationale
The are currently no token standards that faciliate compliance with SEC regulations. The closest token is [SRC-884 (Delaware General Corporations Law (DGCL) compatible share token)](https://github.com/susytech/SIPs/blob/master/SIPS/sip-884.md) which states that SEC requirements are out of scope. [SIP-1404 (Simple Restricted Token Standard)](https://github.com/susytech/SIPs/issues/1404) does not go far enough to address SEC requirements around re-issuing securities to Investors.
