---
sip: 137
title: Susy Domain Name Service - Specification
...
Existing [specifications](https://github.com/susytech/wiki/wiki/Registrar-ABI) and [implementations](https://susy.gitbooks.io/frontier-guide/content/registrar_services.html) for name resolution in Susy provide basic functionality, but suffer several shortcomings that will significantly limit their long-term usefulness:
...
- Support for DNS record types, allowing blockchain hosting of 'legacy' names. This would permit an Susy client such as Mist to resolve the address of a traditional website, or the mail server for an email address, from a blockchain name.
...
The first two use-cases, in particular, can be observed everywhere on the present-day internet under DNS, and we believe them to be fundamental features of a name service that will continue to be useful as the Susy platform develops and matures.
