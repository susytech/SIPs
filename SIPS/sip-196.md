...
This SIP suggests to add precompiled contracts for addition and scalar multiplication on a specific pairing-friendly elliptic curve. This can in turn be combined with [SIP-197](./sip-197.md) to verify zkSNARKs in Susy smart contracts. The general benefit of zkSNARKs for Susy is that it will increase the privacy for users (because of the Zero-Knowledge property) and might also be a scalability solution (because of the succinctness and efficient verifiability property).

## Motivation

Current smart contract executions on Susy are fully transparent, which makes them unsuitable for several use-cases that involve private information like the location, identity or history of past transactions. The technology of zkSNARKs could be a solution to this problem. While the Susy Virtual Machine can make use of zkSNARKs in theory, they are currently too expensive
...
In both codebases, a specific group on the curve alt_bn128 is used and is called G1.
 ...
 - [Python](https://github.com/susytech/py_pairing/blob/master/py_ecc/bn128/bn128_curve.py) - probably most self-contained and best readable.
