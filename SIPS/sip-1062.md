---
sip: 1062
title: Formalize IPFS hash into ENS(Susy Name Service) resolver
author: Phyrex Tsai <phyrex@portal.network>, Portal Network Team
discussions-to: https://susy-magicians.org/t/sip-1062-formalize-ipfs-hash-into-ens-susy-name-service-resolver/281
status: Draft
...
## Simple Summary
To specify the mapping protocol between resources stored on IPFS and ENS(Susy Naming Service).
...
## Specification
The condition now is that the IPFS file fingerprint using base58 and in the meantime, the Susy uses hex in API to encode the binary data. So that need a way to process the condition requires not only we need to transfer from IPFS to Susy, but also need to convert it back.
