...
Abstract
========
This draft SIP describes the details of an authorization method that if provided by rpc enabled susy nodes would allow regular websites to send transactions (via ```eth_sendTransaction```) without the need to enable CORS. Instead, user would be asked to confirm the transaction via an html popup.
...
<html>
  <head>
    <title>Susy Authorization</title>
  </head>
  <script>
