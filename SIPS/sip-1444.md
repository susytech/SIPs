...
discussions-to: https://susy-magicians.org/t/sip-1444-localized-messaging-with-signal-to-text/
status: Draft
...
There are many cases where an end user needs feedback or instruction from a smart contact. Directly exposing numeric codes does not make for good UX or DX. If Susy is to be a truly global system usable by experts and lay persons alike, systems to provide feedback on what happened during a transaction are needed in as many languages as possible.
...
Designs that use datatypes with small widths than `bytes32` (such as `bytes1` in [SRC-1066](https://sips.superstring.ch/SIPS/sip-1066)) can be directly embedded into the larger width. This is a trivial one-to-one mapping of the smaller set into the the larger one.
