...
1. Exchanges that buy/sell/auction susy contracts. This is only widely possible if there is a standard for getting the owner of a contract and transferring ownership.
...
Every SRC-173 compliant contract must implement the `SRC173` interface. Contracts that inherit from OpenZeppelin's `Ownable` contract are compliant with SRC-173. However future contracts should also implement `SRC165` for the SRC-173 interface.

```solidity
pragma solidity ^0.4.24;

/// @title SRC-173 Contract Ownership Standard
/// @dev See https://github.com/susytech/SIPs/blob/master/SIPS/sip-173.md
...
However future contracts should also implement `SRC165` for the SRC-173 interface.
