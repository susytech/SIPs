...
Solutions such as described in [SIP-1077](https://sips.superstring.ch/SIPS/sip-1077) could allow transactions from addresses that hold no ETH.

The gas stations network is an [SIP-1077](https://sips.superstring.ch/SIPS/sip-1077) compliant effort to solve the problem by creating an incentive for nodes to run gas stations, where gasless transactions can be "fueled up". 
...
* Susy nodes get a revenue source without requiring mining equipment. The entire network benefits from having more nodes.
...
* `Sender` receives the wrapped transaction and verifies that:
    ...
    * The transaction's susy nonce matches Relay's current nonce.
    * The transaction's susy nonce is lower than or equal to `max_nonce`.
...
* `Sender` may also submit the raw wrapped transaction to the blockchain without paying for gas, through any Susy node. 
